...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [ApplicationDelegate.descargasEngine descargarImagenConURL:_wallpaper.imagenURL onCompletion:^(UIImage * imagen){
        
        UIImage *img = [[UIImage imageWithData:UIImageJPEGRepresentation(imagen, 1.0f)] resizedImageWithContentMode:UIViewContentModeScaleAspectFit bounds:_imageView.bounds.size interpolationQuality: kCGInterpolationHigh];
...
//        NSString *body = [NSString stringWithFormat:@"A continuación encuentra los datos del solicitante:\n-nombre: %@\n-apellido: %@\n-mail: %@\n-telefono: %@\n-ciudad: %@\n-carro: %@\n",nombre, apellido, mail, telefono, ciudad, carro];
        ...
        [mailer addAttachmentData:UIImageJPEGRepresentation(_imageView.image, 1.0) mimeType:@"image/jpeg" fileName:_wallpaper.nombre];
